                ...
                @auth
                    <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                        ...
                        @if(Auth::user()->role === 'aluno')
                            <x-nav-link :href="route('students.history')" :active="request()->routeIs('students.history')">
                                {{ __('Meu Histórico') }}
                            </x-nav-link>
                        @endif
                        ...
                        @if(in_array(Auth::user()->role, ['admin', 'coordenador']))
                            ...
                            <x-nav-link :href="route('coordinators.index')" :active="request()->routeIs('coordinators.*')">
                                {{ __('Coordenadores') }}
                            </x-nav-link>
                        @endif
    ...
    @auth
        <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
            <div class="pt-2 pb-3 space-y-1">
                ...
                {{-- ===== INÍCIO DA ALTERAÇÃO ===== --}}
                {{-- LINKS DE NAVEGAÇÃO PARA O MENU RESPONSIVO (MOBILE) --}}
                @if(Auth::user()->role === 'aluno')
                    <x-responsive-nav-link :href="route('students.history')" :active="request()->routeIs('students.history')">
                        {{ __('Meu Histórico') }}
                    </x-responsive-nav-link>
                @endif
                ...
                @if(in_array(Auth::user()->role, ['admin', 'coordenador']))
                    ...
                    <x-responsive-nav-link :href="route('coordinators.index')" :active="request()->routeIs('coordinators.*')">
                        {{ __('Coordenadores') }}
                    </x-responsive-nav-link>
                @endif
