<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-headline leading-tight">
            Editar Professor: <span class="text-primary">{{ $teacher->user->name }}</span>
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-background overflow-hidden shadow-xl sm:rounded-lg">
                <form action="{{ route('teachers.update', $teacher) }}" method="POST" class="p-6 sm:p-8">
                    @csrf
                    @method('PUT')

                    {{-- O parcial 'teachers.form' deve ser o estilizado que já corrigimos --}}
